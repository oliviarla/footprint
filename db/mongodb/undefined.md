...
* 트랜잭션이 커밋되어야 변경된 내용들이 실제 데이터베이스에 반영된다.
...
* 스냅샷으로 인한 리소스 소모
  * 트랜잭션이 많고 길어질수록 WiredTiger 엔진이 유지해야 할 스냅샷 데이터가 많아져 메모리와 스토리지 부하가 증가할 수 있다.

## 분산 트랜잭션

* WiredTiger MVCC
  * MVCC란 Multi-Version Concurrency Control의 약자로, 데이터를 수정할 때 원본을 덮어쓰는 것이 아닌 새로운 버전을 만들어 관리하는 방식이다.
* Sharded Cluster 환경의 경우 트랜잭션 사용 시 여러 샤드 간 원자성을 보장하기 위해 2PC(Two Phase Commit)를 사용한다.
  * Transaction Coordinator: 각 샤드에 prepare 요청을 보내고 모두 가능하다는 응답을 받은 경우에만 트랜잭션을 커밋한다. 만약 하나의 노드라도 실패 응답을 보내거나 응답이 없다면 롤백한다. 주로 쿼리를 받는 `mongos`가 이 역할을 담당한다.
  * Participants: 실제 데이터가 저장된 각 `mongod` 가 이 역할을 담당한다. 로컬 트랜잭션을 수행하고 결과를 Coordinator에 전달한다.
