...
*   웹 관련 공통사항을 처리하기 때문에 파라미터로 ServletRequest와 ServletResponse 객체가 제공된다.
    ...
    > 스프링 프레임워크는 확장성을 위해 ServletRequest와 ServletResponse 객체로 제공했지만 대부분 HttpServletRequest와 HttpServletResponse를 사용하기 때문에 다운 캐스팅하여 사용하면 된다.
* 메서드
...
* Dispatcher란 배치 담당자 라는 뜻을 가지며, 그 뜻에 맞게 들어오는 **모든 Request를 우선적으로 받아 처리해주는 서블릿**이다.
...
* 웹 관련 공통 관심 사항을 처리한다는 점에서는 필터와 비슷하다.
* 필터와 유사하게 체인으로 구성되며 중간에 인터셉터를 자유롭게 추가/제거 가능하다.
...
* afterCompletion
  * 모든 작업(뷰 렌더링)이 완료된 후 실행된다.
  * try catch finally 절에서 finally처럼 무조건 호출된다.
